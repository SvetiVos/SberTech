...
public class NoteController {
    ...
    @GetMapping("/filter")
    public String filterNotes(@RequestParam(name = "status", required = false) String status, Model model) {
        List<Note> notesList;
        if ("completed".equals(status)) {
            notesList = notesService.filterNotesByStatus(true);
        } else if ("not_completed".equals(status)) {
            notesList = notesService.filterNotesByStatus(false);
        } else {
            notesList = notesService.listNotes(null);
        }
        model.addAttribute("notesList", notesList);
        return "notesList";
    }

    @PostMapping("/note/updateStatus/{id}")
    public String updateNoteStatus(@PathVariable Long id, @RequestParam(name = "status") String status) {
        Boolean completedStatus = "completed".equals(status);
        notesService.updateNoteStatus(id, completedStatus);
        return "redirect:/";
    }
